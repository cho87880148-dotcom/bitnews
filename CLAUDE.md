...
## ★ 되돌려야 할 임시 변경이 걸려 있다 (2026-08-06 ~)

광고 승인을 받으려고 **첫 화면(`dist/index.html`)에만** 상호를 `Novacent` 로 바꾸고
사업자 정보를 넣어뒀다. 나머지 889개 페이지는 그대로 `비트뉴스` 다.

**승인되면 `build.ps1` 의 `$tempHomeBrand` / `$tempHomeBizInfo` 두 줄을 `''` 로 바꾸고
빌드 후 push 하면 원상복구된다.** 자세한 내용은 `worklog/2026-08-06.md`.
